...
### 6、Redis 秒杀 ，商品库存队列用一个 String 存一个数字，通过数字的递减来减少库存

### 7、Redis 的 List 是链表结构？

### 8、Redis 有哪些数据类型？

### 9、Memcached 和 Redis 的区别？

### 10、数据库引擎 MyISAM 为什么比 InnoDB 快？MyISAM 的存储结构？
...
### 14、ThinkPHP 和 Laravel 的比较？

### 15、Laravel 的中间件 和 过滤器的区别？

### 16、Laravel 中间件 的前置处理，后置处理 

### 17、Laravel 的 IoC 和 DI ，DI 的实现原理 
